...
namespace XMLDataBase
{
    public static class ObjectLogic
    {
        ...
        private static object GetItem(OBJS.Data Data, Type NewObject)
        {
            ...
            /*
            object Item = Activator.CreateInstance(NewObject);
            List<PropertyInfo> Props = GetProprites(OBJS.Types.Prop, NewObject);

            foreach (PropertyInfo Prop in Props)
            {
                string XMLValue = Data.GetValue(Prop.Name);
                if (XMLValue == "")
                {
                    continue;
                }
                ...
                try
                {
                    object Value = GetValue(Prop.PropertyType, XMLValue);
